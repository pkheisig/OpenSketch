...
describe("canvas wheel zoom", () => {
  it("uses 0.0075 gesture sensitivity and fixed three-percent wheel steps", () => {
    expect(GESTURE_ZOOM_SENSITIVITY).toBe(0.0075);
    expect(WHEEL_ZOOM_STEP).toBe(0.03);
    expect(wheelZoomDelta(-1)).toBe(0.0075);
    expect(wheelZoomDelta(1)).toBe(-0.0075);
    expect(wheelZoomDelta(-100)).toBe(0.03);
    expect(wheelZoomDelta(100)).toBe(-0.03);
    expect(wheelZoomDelta(-1, WheelEvent.DOM_DELTA_LINE)).toBe(0.03);
    expect(wheelZoomDelta(1, WheelEvent.DOM_DELTA_LINE)).toBe(-0.03);
