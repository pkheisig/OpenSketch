...
function projectRepository(): ProjectRepository {
  return {
    list: listProjects,
    get: getProject,
    subscribeChanges: subscribeProjectChanges,
    save: async (project) => {
      const result = await saveProject(project);
      if (result.status === "conflict") throw new ProjectConflictError(result.current);
      project.revision = result.project.revision;
      project.updatedAt = result.project.updatedAt;
    },
    saveThumbnail: async (id, revision, thumbnail) => {
      const current = await getProject(id);
      if (!current || String(current.revision) !== revision) return current;
      return saveProjectThumbnail(id, current.revision ?? 0, thumbnail);
    },
    create: createProject,
    delete: async (id, expectedRevision) => {
      const current = await getProject(id);
      const result = await deleteProject({ id, revision: expectedRevision ?? current?.revision });
      if (result.status === "conflict") throw new ProjectConflictError(result.current);
    },
    duplicate: duplicateProject,
    moveToFolder: async (project, folderId) => {
      const result = await moveProjectToFolder(project, folderId);
      if (result.status === "conflict") throw new ProjectConflictError(result.current);
    },
