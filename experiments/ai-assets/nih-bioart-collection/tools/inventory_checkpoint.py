...
inventory_complete=len(complete)==len(data["assets"])
...
if inventory_complete:
    rows=[row.replace("Completed assets are pushed periodically. The full inventory is still in progress.", "The complete concrete inventory has passed asset validation and visual review. PNGs, editable SVGs, originals and evidence are retained in separate folders.").replace("The thread continuation resumes this ledger until all entries are covered.", "All concrete checklist entries are covered; no production items remain pending.") for row in rows]
